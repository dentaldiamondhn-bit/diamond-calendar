...
> **Last updated:** 2026-09-14 · **Status:** Phases 0–5 complete (Phase 5 live QA passed 2026-09-13) · **Phase 6 ✅ — Android/PWA Standalone App PWA-parity ported 2026-09-13/14 (commits `7bac426`/`1f7b649`/`ea18f49`/`4fe3972`); Capacitor APK restored with native FCM push (commit `2c0b5b1`)** · **Phase 9 ~40%** · Phases 7–8 pending · Phase 10 ledger-only start
>
...
| **6** | Android/PWA Standalone App (`diamond-calendar` sync) | ✅ | `████████████████████ 100%` | PWA-parity port done (see Phase 6): RBC engine + data/push/SSE layers byte-identical to web (`7bac426` ×89 +9540/−5858), layout + branding/icons fixed; env service-role key converged valid across all 4 env files | **✅ Executed 2026-09-13/14.** Native Capacitor APK restored with FCM push via firebase-admin (`2c0b5b1`): `@capacitor/push-notifications|local-notifications|app-launcher`, `src/lib/firebaseAdmin.ts`, `NativePushListener`, `capacitor.config.ts` + `server.url` = live PWA. **User must:** (a) run `add_push_subscriptions_fcm.sql` migration in Supabase SQL editor; (b) set `FIREBASE_SERVICE_ACCOUNT` + VAPID/push/cron env vars in Vercel dashboard. See Phase 6 section |
...
| **6 – Android/PWA Standalone App** | ✅ | `████████████████████ 100%` | PWA-parity port done; native Android glue removed 2026-07-27 (`6fbf1d4`), **restored + FCM added 2026-09-14** | **✅ Executed 2026-09-13/14:** PWA-parity port (`7bac426`/`1f7b649`/`ea18f49`/`4fe3972`) + native APK restore with FCM (`2c0b5b1`): `@capacitor/push-notifications|local-notifications|app-launcher`, `src/lib/firebaseAdmin.ts` (firebase-admin v14 modular), `PushService` FCM routing, `usePushNotifications` native branch, `NativePushListener` (foreground re-present + tap deep-link), `subscribe/route.ts` Capacitor path (`fcm:<token>`), Android manifest `POST_NOTIFICATIONS` + `SCHEDULE_EXACT_ALARM`, `default_notification_channel_id` = `reminders`, `notification_icon.xml` + `colors.xml`, debug APK rebuilt (10.6 MB, FCM libs merged). **User must:** (a) run `add_push_subscriptions_fcm.sql` migration; (b) set `FIREBASE_SERVICE_ACCOUNT` + `NEXT_PUBLIC_VAPID_PUBLIC_KEY_1`, `VAPID_PRIVATE_KEY_1`, `PUSH_WEBHOOK_SECRET`, `CRON_SECRET` in Vercel. |
...
**Android APK restore (2026-09-14 — done ✅, commit `2c0b5b1`):**
...
- Restore scope: `@capacitor/core|cli|android` 8.x + `@capacitor/push-notifications@8.1.2` +
  `@capacitor/local-notifications@8.3.1` + `@capacitor/app-launcher@8.0.1` +
  `firebase-admin@14.4.0`; `capacitor.config.ts` (`appId com.diamondcalendar.app`, `webDir out`,
  **`server.url https://calendario.dentaldiamondhn.com`**, `androidScheme https`).
- `npx cap add android` → `npx cap sync android` → `gradlew assembleDebug` →
  `android/app/build/outputs/apk/debug/app-debug.apk` (10.6 MB, debug-signed, targetSdk 36).
- Android SDK re-installed (none present): cmdline-tools + `platforms;android-36` + `build-tools;36.0.0` +
  `platform-tools`; JDK21 + gradle 8.14.3 wrapper.
- **Native push notification fix** — the Capacitor WebView doesn't support the Web Push API
  (`PushManager`) or Web Notifications API, so the APK needs native FCM via `@capacitor/push-notifications`
  + `firebase-admin`:
  - **Server:** `src/lib/firebaseAdmin.ts` (firebase-admin v14 modular API, `sendFCMNotification`
    with channel `reminders`, icon/color/tag mapping, `unregistered`-token detection);
    `src/lib/push/pushService.ts` splits web vs native subscribers and fans FCM out per-platform.
  - **Client:** `hooks/usePushNotifications.ts` gains a native branch (`PushNotifications.register` +
    `LocalNotifications.createChannel` with foreground re-present + tap deep-link);
    `components/calendar-new/NativePushListener.tsx` (mounted in calendario page);
    `PushStatusBadge` shows native state without swReady gate.
  - **API:** `app/api/push/subscribe/route.ts` accepts `{ platform: 'capacitor', fcmToken }` and
    upserts an `fcm:<token>` endpoint row.
  - **Android:** `google-services.json` (copied to `android/app/`), `POST_NOTIFICATIONS` +
    `SCHEDULE_EXACT_ALARM` perms, `FirebaseMessagingService` wired via push plugin manifest,
